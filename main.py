...
class transaction:
    ...
    def blur_from(self):
        if self.trans_from == 'Данные об отправителе отсутствуют':
            ...
            card_number = self.trans_from[len(self.trans_from)-16:len(self.trans_from)]
    ...
    def blur_to(self):
        if self.trans_desc == 'Перевод организации' or self.trans_desc == 'Открытие вклада' or self.trans_desc == 'Перевод со счета на счет':
            card_number = self.trans_to[len(self.trans_to) - 20:len(self.trans_to)]
        else:
            card_number = self.trans_to[len(self.trans_to) - 16:len(self.trans_to)]
